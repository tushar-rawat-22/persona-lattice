...
def _query(kind: str = "url", value: str = "https://www.wikidata.org/wiki/Q42") -> ProviderQuery:
    return ProviderQuery(subject_id=uuid4(), identifier_id=uuid4(), identifier_kind=kind, identifier_value=value)
...
async def test_success_retains_only_bounded_cc0_metadata_and_emits_no_leads() -> None:
    ...
    extraction = extract_observation_leads(details=observation.payload, source="wikidata_exact_entity", source_locator=observation.source_locator)
...
async def test_missing_entity_is_completed_zero_observation_result() -> None:
    async def fetcher(entity_id: str) -> dict[str, object]:
        return {"entities": {entity_id: {"id": entity_id, "type": "item", "missing": ""}}}
...
async def test_provider_rejects_secret_wrong_kind_and_non_entity_url() -> None:
    provider = WikidataExactEntityProvider(fetcher=lambda entity_id: None)  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_mismatched_or_malformed_entity_fails_closed() -> None:
    async def mismatch(entity_id: str) -> dict[str, object]:
        return {"entities": {entity_id: {"id": "Q99", "type": "item", "labels": {"en": {"value": "X"}}}}}
...
class _Response:
    ...
    def read(self, size: int) -> bytes: return self.raw[:size]
...
def test_transport_maps_429_transient_and_oversized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def rate_limited(request, timeout: float): raise HTTPError(request.full_url, 429, "limited", headers, None)
    monkeypatch.setattr(wikidata, "urlopen", rate_limited)
    with pytest.raises(ProviderRemoteRateLimitError) as exc_info: wikidata._fetch_wikidata_entity_sync("Q42")
    assert exc_info.value.retry_after == 7.0
    def unavailable(request, timeout: float): raise HTTPError(request.full_url, 503, "down", Message(), None)
    monkeypatch.setattr(wikidata, "urlopen", unavailable)
    with pytest.raises(ProviderTransientError): wikidata._fetch_wikidata_entity_sync("Q42")
    monkeypatch.setattr(wikidata, "urlopen", lambda request, timeout: _Response(b"x" * (wikidata._MAX_RAW_RESPONSE_BYTES + 1)))
    with pytest.raises(ProviderResponseTooLarge): wikidata._fetch_wikidata_entity_sync("Q42")
