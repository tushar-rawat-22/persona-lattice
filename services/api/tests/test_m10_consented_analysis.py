...
from app.intelligence.m10_consented_analysis import (
    M10CountFraction,
...
def _provenance(fixtures, *, basis: M10LabelBasis):
    return tuple(
        M10FixtureLabelProvenance(
            ...
            basis=basis,
            evidence_digest=_digest(f"contract-test-external-record:{fixture.name}:v1"),
        )
...
def test_consented_analysis_uses_scenario_specific_admitted_denominators() -> None:
    fixtures = broadened_synthetic_m10_cohort()
    analysis = build_m10_consented_cohort_analysis(
        fixtures=fixtures,
        replay=_replay(fixtures),
        provenance=_provenance(fixtures, basis=M10LabelBasis.CONSENTED),
    )
...
def test_consented_analysis_rejects_synthetic_or_mixed_provenance() -> None:
    fixtures = broadened_synthetic_m10_cohort()
    replay = _replay(fixtures)

    with pytest.raises(ValueError, match="rejects synthetic or mixed"):
        build_m10_consented_cohort_analysis(
            fixtures=fixtures,
            replay=replay,
            provenance=_provenance(fixtures, basis=M10LabelBasis.SYNTHETIC),
        )

    mixed = list(_provenance(fixtures, basis=M10LabelBasis.CONSENTED))
    mixed[0] = replace(mixed[0], basis=M10LabelBasis.SYNTHETIC)
    with pytest.raises(ValueError, match="rejects synthetic or mixed"):
        build_m10_consented_cohort_analysis(
            fixtures=fixtures,
            replay=replay,
...
def test_consented_analysis_rejects_unlabelled_admitted_pivots() -> None:
    fixtures = tuple(
...
def test_consented_analysis_is_replay_and_manifest_anchored() -> None:
    fixtures = broadened_synthetic_m10_cohort()
    replay = _replay(fixtures)
    provenance = _provenance(fixtures, basis=M10LabelBasis.CONSENTED)

    forward = build_m10_consented_cohort_analysis(
        fixtures=fixtures,
        replay=replay,
        provenance=provenance,
    )
    reverse = build_m10_consented_cohort_analysis(
