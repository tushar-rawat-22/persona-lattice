...
def test_shared_production_runtime_owns_current_governed_quick_research_providers() -> None:
    ...
    assert DEFAULT_PROVIDER_RUNTIME.adapters["sec_edgar_exact_cik"] is DEFAULT_SEC_EDGAR_PROVIDER
...
def test_shared_runtime_adapters_match_reviewed_registry_descriptors() -> None:
    ...
    assert DEFAULT_SEC_EDGAR_PROVIDER.descriptor is PROVIDER_BY_NAME["sec_edgar_exact_cik"]
...
def test_default_provider_returns_process_owned_adapter_without_reinstantiation() -> None:
    ...
    assert default_provider("sec_edgar_exact_cik") is DEFAULT_SEC_EDGAR_PROVIDER
