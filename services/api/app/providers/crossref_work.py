...
def _validated_doi(value: str) -> str | None:
    if not value or len(value) > _MAX_DOI_LENGTH:
        return None
...
def crossref_doi_from_url(value: str) -> str | None:
    """Return the DOI carried by one canonical ``https://doi.org/...`` URL.

    Crossref is never used as a DOI search engine. A network call is applicable
    only when the operator already supplied an exact DOI resolver URL.
    """
...
def _work_from_payload(payload: dict[str, object], *, expected_doi: str) -> dict[str, object]:
    ...
    canonical_doi = _validated_doi(returned_doi.strip())
