...
_USER_AGENT = "PersonaLattice/0.0.1 (https://github.com/tushar-rawat-22/persona-lattice)"
...
def _fetch_datacite_doi_sync(doi: str) -> dict[str, object] | None:
    # DataCite documents singleton DOI paths with DOI slashes left as path
    # separators. Quote query/fragment-sensitive punctuation but keep slashes.
...
def _publication_year(attributes: dict[str, object]) -> int | None:
    ...
    if value is None:
        return None
    if isinstance(value, bool) or not isinstance(value, int) or value < 1000 or value > 9999:
...
def _creator_names(attributes: dict[str, object]) -> tuple[str, ...]:
    ...
    for creator in creators[:_MAX_CREATOR_COUNT]:
        ...
        if name is None:
            ...
            parts = [part for part in (given, family) if isinstance(part, str) and part.strip()]
            if not parts:
                continue
            name = " ".join(parts)
...
def _record_from_payload(payload: dict[str, object], *, expected_doi: str) -> dict[str, object]:
    ...
    if state is not None and state != "findable":
        raise ProviderResultValidationError("DataCite public DOI is not in findable state.")

    returned_doi = attributes.get("doi")
    if not isinstance(returned_doi, str):
        returned_doi = data.get("id")
    if not isinstance(returned_doi, str):
        raise ProviderResultValidationError("DataCite DOI response is missing its DOI.")
...
class DataCiteExactDoiProvider:
    ...
    async def execute(self, query: ProviderQuery, secret: str | None) -> ProviderResult:
        ...
        doi = doi_from_canonical_url(query.identifier_value)
        if doi is None:
        ...
        details = _record_from_payload(payload, expected_doi=doi)
