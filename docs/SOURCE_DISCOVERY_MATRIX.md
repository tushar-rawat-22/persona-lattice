...
| Mastodon / ActivityPub WebFinger | Free protocol path on the target host; no generic API key required for WebFinger | Resolves an already-known `username@domain` / `acct:` identity to the canonical ActivityPub actor and profile URI | A bare handle is ambiguous across instances, and broad instance search/follower harvesting would become enumeration rather than verification | **High-value review after LC1.** Exact federated identifier only; no bare-handle guessing, instance-wide search, follower/following enumeration or post harvesting |
...
| Stack Exchange public user API | Free public API; unauthenticated applications share an IP/key daily quota | High-quality public technical-profile metadata when an exact Stack Exchange user ID or canonical profile URL is already known | `/users?inname=` is substring search and can return many unrelated people; network-wide/name searching would create weak identity matches | **Medium-priority review after LC1.** Prefer exact user ID / canonical profile URL. Do not promote display-name substring search into an identity lookup |
| Hacker News official API | Free, public, read-only Firebase API; no authentication and currently no documented rate limit | Exact case-sensitive public username lookup with account creation time, karma and optional public self-description; only users with public activity are returned | The API also exposes submitted item IDs and live update feeds, which would create unnecessary content harvesting or broad enumeration if adopted wholesale | **High-value review after LC1.** Exact known username or canonical profile only; retain minimal public profile fields. Do not enumerate updates, traverse submissions, or infer identity from karma/activity |
| ORCID Public API | Public access exists, but ORCID states free Public API use is for non-commercial / non-revenue-generating services | Rich scholarly affiliation and works metadata for an exact known ORCID iD | PersonaLattice is intended to become a commercial product; using the free public API would conflict with the zero-spend commercial baseline, and ORCID data must not be treated as automatic identity verification | **Reject for zero-spend commercial runtime.** Revisit only if commercial terms later fit; no registry scraping workaround |
...
- Stack Exchange API `/users` and `/users/{ids}`: https://api.stackexchange.com/docs/users and https://api.stackexchange.com/docs/users-by-ids
...
- Y Combinator Hacker News API announcement: https://www.ycombinator.com/blog/hacker-news-api
- ORCID publisher/public-access guidance: https://info.orcid.org/orcid-for-publishers/
- ORCID visibility guidance: https://support.orcid.org/hc/en-us/articles/360006897614-Visibility-settings
