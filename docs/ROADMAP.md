...
Retained evidence is limited to the canonical QID, bounded English label/description when present, CC0 attribution and `identity_claim=false`. It does not retain claims, aliases, sitelinks, external IDs, dates, locations, occupations, organizations, contacts or linked entities and emits no recursive leads.

The source is credentialless and zero-direct-cost. Requests carry a meaningful PersonaLattice User-Agent, use a one-concurrency local budget of 30 requests/minute, and preserve provider `429`/`Retry-After`. Returned entity-ID mismatch, non-item results and malformed provider data fail closed rather than silently changing entity context.
