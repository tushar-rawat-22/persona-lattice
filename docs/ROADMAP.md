...
Completed source-state/report work:
...
1. deterministic synthetic/failure fixtures over source evaluation counters;
2. explicit typed pre-execution/configuration/malformed-result outcomes only where the runtime can prove them;
...
7. run architecture consistency and graph-growth evaluation before activating new network providers.

No new third-party source should be activated during these closure blocks.

## M10 — evaluation and calibration laboratory

**Status: started at the descriptive-counter layer; full evaluation follows V2-D closure**

Required before increasing recursion limits or changing correlation thresholds:

- deterministic synthetic/failure source fixtures;
...
Build deterministic failure/evaluation fixtures on top of the new source counters, then add graph-growth/wrong-pivot/duplicate measurements. After that, finish the remaining existing-source migration and document-to-reviewed-lead plumbing.
