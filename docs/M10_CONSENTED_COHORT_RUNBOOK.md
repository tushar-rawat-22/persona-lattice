...
M10 also has an `independently_reviewed` provenance basis, but that is deliberately separate. Do not use this runner for reviewed-but-not-consented evidence. Use the reviewed runner and its external review record instead.
...
The JSON file cannot choose its own provenance basis. `consented` is fixed by the runner code; any `basis` or `label_basis` field is rejected.
