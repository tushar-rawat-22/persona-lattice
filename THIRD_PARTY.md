...
| GitHub REST API | GitHub Terms of Service / API Terms | Active for exact public username profiles and exact canonical public repository URLs through one shared unauthenticated provider budget. Repository lookup uses only `GET /repos/{owner}/{repo}` and retains bounded repository identity/state metadata; it does not search repositories/code/topics, enumerate owners or members, fetch contributors/commits/issues/releases/contents, or emit repository-owner leads. No GitHub code or dataset is vendored. |
| GitLab REST API | GitLab Terms of Use / Acceptable Use Policy / API documentation | Active for exact public username and public-email profile matches plus exact two-segment `gitlab.com/<namespace>/<project>` public project URLs through one shared unauthenticated provider budget. Project lookup uses only `GET /api/v4/projects/{URL-encoded path}` and retains bounded project/namespace state metadata; it does not search projects/groups, enumerate owners or members, fetch repository content/contributors/commits/issues/MRs/releases/pipelines/packages, or emit project leads. Subgroup project URLs remain intentionally inapplicable in this first version. No GitLab code or dataset is vendored. |
