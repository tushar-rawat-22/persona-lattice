...
assert.ok(
  source.includes('const sourceCases = remoteFiltering && !remoteSearchFailed ? remoteCases : cases;'),
  "remote results must replace loaded-only filtering only while a retained-index filter is active",
);
assert.ok(
