...
function SourceRunSummary({ sourceRuns, title }: { sourceRuns?: SourceRunReport; title: string }) {
  ...
  return (
    <div className="reportSection">
      <h3>{title}</h3>
      <div className="sourceSummaryLine">
        <span>{sourceRuns.record_count} records</span>
      ...
      {sourceRuns.records.length === 0 ? (
        ...
        <div className="tableScroll">
          <table className="compactTable sourceTable">
            <thead><tr><th>Source</th><th>State</th><th>Kind</th><th>Attempt</th><th>Obs.</th><th>Reason</th></tr></thead>
            <tbody>
              {sourceRuns.records.map((item, index) => (
                <tr key={`${item.source}-${item.lead_kind}-${index}`}>
                  <td>{item.source}</td>
                  <td><span className={`statePill state-${item.state}`}>{item.state}</span></td>
                  <td>{item.lead_kind}</td>
                  <td>{item.execution_attempted ? "yes" : "no"}</td>
      ...
      {missingEvidenceReasons.length > 0 && (
        <details>
          <summary>Why evidence may be missing</summary>
          <ul className="coverageList">
...
function M5EvidenceTable({ report }: { report: ConvergedReport }) {
  if (report.m5.evaluations.length === 0) {
    return (
      <div className="reportSection">
...
function ConvergedSources({ report }: { report: ConvergedReport }) {
  ...
  return (
    <div className="reportSection">
      ...
      <div className="tableScroll">
        <table className="compactTable sourceTable">
          <thead><tr><th>Node</th><th>Source</th><th>State</th><th>Attempt</th><th>Obs.</th><th>Reason</th></tr></thead>
          <tbody>
            {rows.map((row, index) => (
              <tr key={`${row.node_key}-${row.source}-${index}`}>
                <td>{row.node_value}</td>
                <td>{row.source}</td>
                <td><span className={`statePill state-${row.state}`}>{row.state}</span></td>
                <td>{row.execution_attempted ? "yes" : "no"}</td>
...
export function QuickResearch({ csrfToken, onActiveCaseChange }: QuickResearchProps) {
  ...
  async function openCase(caseId: string) {
    ...
    try {
      ...
      if (isCurrentCaseContext(generation)) setError("Stored case could not be loaded.");
    }
  }
  ...
  async function loadOlderCases() {
    ...
    try {
      ...
      if (isCurrentCaseList(generation)) setError("Older stored cases could not be loaded.");
    } finally {
      if (isCurrentCaseList(generation)) setLoadingOlderCases(false);
    }
  }
  ...
  function handleTabKeyDown(event: KeyboardEvent<HTMLButtonElement>, view: WorkspaceView) {
    ...
    event.preventDefault();
    const target = WORKSPACE_VIEWS[targetIndex];
    setActiveView(target);
    document.getElementById(`case-tab-${target}`)?.focus();
  }
        ...
        <div className="reportSummary">
          ...
          {report.seed_provenance && (
            <div className="reportSection">
              ...
              <p className="muted">
                {report.seed_provenance.human_reviewed ? "Human reviewed" : "Review state unavailable"} · {report.seed_provenance.source} · <SourceLocator locator={report.seed_provenance.source_locator} />
              </p>
            </div>
          )}
          <div className="reportSection">
          ...
          <M5EvidenceTable report={converged} />
          {converged.warnings.length > 0 && (
            <details><summary>{converged.warnings.length} research warnings</summary><ul className="coverageList">{converged.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></details>
          )}
        </div>
      );
    }
    if (structured) {
      return (
        <div className="reportSummary">
          ...
          {structured.coverage_gaps.length > 0 && <ul className="coverageList">{structured.coverage_gaps.map((gap) => <li key={gap}>{gap}</li>)}</ul>}
        </div>
      );
    }
    ...
    if (converged) {
      ...
      return (
        <div className="reportSection">
          ...
          <div className="tableScroll">
            <table className="compactTable pivotTable">
              <thead><tr><th>From</th><th>To</th><th>Reason</th><th>Provenance</th></tr></thead>
              <tbody>
          ...
          <p className="muted">A pivot is a lead to investigate, not proof that two identifiers belong to the same person.</p>
        </div>
      );
    }
    if (structured) {
      ...
      return (
        <div className="reportSection">
          ...
          <div className="tableScroll">
            <table className="compactTable">
              <thead><tr><th>Kind</th><th>Value</th><th>Source</th><th>Locator</th></tr></thead>
              <tbody>
                {structured.connected_identifiers.map((item, index) => {
                  const resolved = resolveConnectedIdentifier(report, item);
                  return <tr key={`${item.kind}-${index}`}><td>{item.kind}</td><td>{resolved?.value ?? "Reference unavailable"}</td><td>{resolved?.source ?? "—"}</td><td>{resolved ? <SourceLocator locator={resolved.source_locator} /> : "Stored connected-field reference could not be resolved safely."}</td></tr>;
                })}
              </tbody>
            </table>
          </div>
  ...
  function renderSources() {
    if (!report) return null;
    if (converged) return <ConvergedSources report={converged} />;
    return <SourceRunSummary sourceRuns={report.source_runs} title="Source execution" />;
  }
        ...
        <div className="quickResult">
          ...
          <div className="workspaceTabs" role="tablist" aria-label="Case workspace views">
            {WORKSPACE_VIEWS.map((view) => (
              <button
                ...
                aria-controls={`case-panel-${view}`}
                tabIndex={activeView === view ? 0 : -1}
                className={activeView === view ? "workspaceTab active" : "workspaceTab"}
          ...
          {WORKSPACE_VIEWS.map((view) => (
            <section
              ...
              key={view}
              role="tabpanel"
              aria-labelledby={`case-tab-${view}`}
              tabIndex={activeView === view ? 0 : -1}
              hidden={activeView !== view}
              className="workspacePanel"
            >
              {activeView === view ? renderView(view) : null}
            </section>
          ))}
        </div>
      )}
